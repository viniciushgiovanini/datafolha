...
class HomePageState extends State<HomePage> {
  ...
  var legendLabels = <String, String>{
  ...
  void initState() {
    ...
    var qtd_lula;
    var qtd_bolsonaro;
    var dado = pegarDado().then((value) => {
          setState(() {
            dadoOfi = jsonDecode(value);
            qtd_lula = dadoOfi["resp"][0]["qtd_votos"].toDouble();
            qtd_bolsonaro = dadoOfi["resp"][1]["qtd_votos"].toDouble();
            dataMap = <String, double>{
              "Lula": qtd_lula,
              "Bolsonaro": qtd_bolsonaro,
            };
          })
        });
  }
...
Future<String> pegarDado() async {
  var dado = await getVoto();
  return dado;
}

Future<String> getVoto() async {
  var uri = Uri.parse("http://localhost:4000/allvotos");
